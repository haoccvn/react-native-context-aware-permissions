...
export const usePermissions = (permissions: PermissionType[]) => {
  ...
  async function checkPermissions(): Promise<Statuses> {
    ...
    if (isIOS) {
      statuses = await checkMultiple(
        [...permissions]
          .map((p) => PERMISSIONS.IOS[p.ios] as unknown as Permission)
          .filter((p) => !!p)
      );
    ...
    if (isANDROID) {
      statuses = await checkMultiple(
        [...permissions]
          .map((p) => PERMISSIONS.ANDROID[p.android] as unknown as Permission)
          .filter((p) => !!p)
      );
      console.log("STATUSES:", statuses);
    }
